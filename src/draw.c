...
void dwDrawPoint(int x, int y, Color col) {
    ...
    if(x < 0 || y < 0 || x >= window_Fb.width || y >= window_Fb.height || window_Fb.channels < 3) 
        return;

    //get a ptr to the 'r' element of the color
    unsigned char* colPtr = &window_Fb.data[(x + y * window_Fb.width) * window_Fb.channels];
    colPtr[0] = col.b;
    colPtr[1] = col.g;
    colPtr[2] = col.r;
    colPtr[3] = 255;
...
void dwDrawRect(int x, int y, int w, int h, Color col) {
    ...
    if(((x + w) <= 0) || ((y + h) <= 0) || (x >= (long)window_Fb.width) || (y >= (long)window_Fb.height)) {
        return;
    }
    ...
    if(x + w >= window_Fb.width) { w = window_Fb.width - x;  }
    if(y + h >= window_Fb.height){ h = window_Fb.height - y; }
...
void dwBlitImage(int x, int y, int w, int h, FilterMode filterMode,  Texture tex){
    ...
    if(mx + w >= window_Fb.width) { mw = window_Fb.width - mx;  }
    if(my + h >= window_Fb.height){ mh = window_Fb.height - my; }
    ...
    for(int i = mx; i < mx+mw; i++) {
        for(int j = my; j < my+mh; j++) {
            ...
            {
            case FM_NEAREST:
                dwDrawPoint(i,j,texGetPixel(round(tx), round(ty), tex));
                break;
            case FM_BILINEAR:
            {
                double x1 = floor(tx); 
                ...
                dwDrawPoint(i,j, (Color){out_r, out_g, out_b});
            }
...
void dwBlitImageToTexture(int x, int y, int w, int h, FilterMode filterMode,  Texture tex, Texture dest){
    //check if the rect is completely off the fb
    int mx = x, my = y;double lerp(double a, double b, double c) {
    return a + (b - a) * c;
}
    int mw = w, mh = w;
